...
#define LEN_REG   2                          /* ceil(log2(NUM_REG)) -- update manually when changing N_CITIES */
#define N_QUBITS  (NUM_REG * LEN_REG)
#define DEPTH     1
#define OPTIMUM   80.0
#define N_BUBBLE_OPS  (NUM_REG * (NUM_REG - 1) / 2)
#define N_PAR         (N_BUBBLE_OPS * DEPTH)
...
/* Symmetric distance matrix for 4 cities (same as tests/data/tsp_4city.dat).
...
/* Bubbles: N_BUBBLE_OPS operators in a bubble-sort sweep pattern. */
static swap_pairs_t bub_ops[N_BUBBLE_OPS];
static qubit_t      bub_qa[N_BUBBLE_OPS * LEN_REG];
static qubit_t      bub_qb[N_BUBBLE_OPS * LEN_REG];

/* Precompute swap pairs for the bubble-sort sweep:
 * outer i = NUM_REG-1 down to 1, inner r = 0 to i-1, swap(r, r+1).
 * Note that swap(0,1) appears twice (in different passes) with different
 * parameters, giving the optimizer more freedom than the adjacency mixer. */
static void build_bub_ops() {
    unsigned idx = 0;
    for (unsigned i = NUM_REG - 1; i >= 1; --i) {
        for (unsigned r = 0; r < i; ++r) {
            unsigned base = idx * LEN_REG;
            for (unsigned b = 0; b < LEN_REG; ++b) {
                bub_qa[base + b] = (qubit_t)(r * LEN_REG + b);
                bub_qb[base + b] = (qubit_t)((r + 1) * LEN_REG + b);
            }
            bub_ops[idx].n_pairs = LEN_REG;
            bub_ops[idx].qa      = &bub_qa[base];
            bub_ops[idx].qb      = &bub_qb[base];
            ++idx;
        }
    }
}
...
/* Bubbles mixer: each precomputed operator is applied with its own parameter. */
static void mixer(state_t *state, const double *params) {
    for (unsigned i = 0; i < N_BUBBLE_OPS; ++i) {
        composite_exp(state, params[i], bub_ops[i].qa, bub_ops[i].qb, bub_ops[i].n_pairs);
    }
}
...
static double obj(unsigned short n, const double *params, void *userdata) {
    ...
    for (unsigned i = 0; i < DEPTH; ++i) {
        mixer(&iota, &params[i * N_BUBBLE_OPS]);
    }
